...
func (rn *RaftNode) becomeLeaderLocked() {
	...
	rn.wg.Add(1)
	go rn.runHeartbeats(rn.currentTerm)
}

// runHeartbeats sends empty AppendEntries to every peer immediately and
// then every HeartbeatInterval, for as long as this node remains leader of
// `term`. The immediate first round matters: it announces the win and
// resets follower timers before any of them can start a competing election.
func (rn *RaftNode) runHeartbeats(term uint64) {
	defer rn.wg.Done()
	ticker := time.NewTicker(rn.cfg.HeartbeatInterval)
	defer ticker.Stop()
	for {
		if !rn.sendHeartbeats(term) {
			return
		}
		select {
		case <-rn.stopCh:
...
// sendHeartbeats fires one round of heartbeats. Returns false when this
// node is no longer leader of `term` and the loop should exit.
func (rn *RaftNode) sendHeartbeats(term uint64) bool {
	rn.mu.Lock()
	...
	args := AppendEntriesArgs{
		Term:     term,
		LeaderID: rn.id,
		// Phase 1 heartbeats carry no entries and followers don't yet run
		// the consistency check; prev fields describe our last entry so
		// the wire traffic is already shaped like Phase 2's.
	...
	for _, peer := range rn.peers {
		go func(peer string) {
			...
			defer rn.mu.Unlock()
			if reply.Term > rn.currentTerm {
				// A partition healed or we were deposed: a node out
				// there has a higher term, so we are not a legitimate
				// leader anymore (§5.1).
				rn.becomeFollowerLocked(reply.Term)
			}
			// Success/failure handling (advancing matchIndex, decrementing
			// nextIndex) arrives with real replication in Phase 2.
		}(peer)
	}
	return true
}
